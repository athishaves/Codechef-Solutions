...
import java.io.*;
  ...
  class AIRLINE {
	...
	public static void func () {
	    int a = sc.nInt(), b = sc.nInt(), c = sc.nInt();
	    int d = sc.nInt(), e = sc.nInt();
	    
	    println(func(a,b,c,d,e) ? "YES" : "NO");
	}
	
	static boolean func (int a, int b, int c, int d, int e) {
	    int sum = a + b + c;
	    if (a <= e && sum - a <= d) return true;
	    if (b <= e && sum - b <= d) return true;
	    if (c <= e && sum - c <= d) return true;
	    return false;
	}
