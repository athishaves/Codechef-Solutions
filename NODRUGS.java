...
import java.io.*;
  ...
  class NODRUGS {
	...
	public static void func () {
	    int n = sc.nInt(), k = sc.nInt(), l = sc.nInt();
	    int[] a = new int[n];
	    
	    println(func(a, n, k, l) ? "Yes" : "No");
	}
	
	static boolean func (int[] a, int n, int k, int l) {
	    ...
	    for (int i=0; i<n; i++) {
	        ...
	        if (a[i] > val) {
	            val = a[i];
	            freq = 1;
	        } else if (a[i] == val) freq++;
	    }
	    
	    if (a[n-1] == val) if (freq == 1) return true;
        
        if (a[n-1] + (l-1)*k > val) return true;
