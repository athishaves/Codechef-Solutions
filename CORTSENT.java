...
import java.io.*;
  ...
  class CORTSENT {
	...
	static boolean check (String a) {
		//println(a);
		if (!valid(a.charAt(0))) return false;
		
		//println(a);
		boolean flag = small(a.charAt(0));
		
		//println(flag);
		for (int i=1; i<a.length(); i++) {
			if (!valid(a.charAt(i), flag)) return false;
		}
		return true;
	}
	
	static boolean small (char a) {
		return a>='a' && a<='z';
	}
	
	static boolean valid (char a) {
		return (a>='a' && a<='m') || (a>='N' && a<='Z');
	}
	
	static boolean valid (char a, boolean flag) {
		if (flag) return (a>='a' && a<='m');
		return  (a>='N' && a<='Z');
	}
