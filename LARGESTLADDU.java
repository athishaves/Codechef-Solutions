...
import java.io.*;
  ...
  class LARGESTLADDU {
	...
	public static void func () {
	    int n = (int) Math.pow(2, sc.nInt());
	    
	    PriorityQueue<Long> q = new PriorityQueue<>();
	    for (int i=0; i<n; i++) q.add(sc.nLong());
	    
	    println(func(q,n) ? "YES" : "NO");
	}
	
	static boolean func (PriorityQueue<Long> q, int n) {
		//pt(q);
		while (n != 1) {
			PriorityQueue<Long> temp = new PriorityQueue<>();
			
			for (int i=0; i<n; i+=2) {
				long x = q.remove(), y = q.remove();
				//println(x + " " + y);
	...
	static void pt (PriorityQueue<Long> q) {
		for (long i : q) print(i + " ");
		println("-----------");
	}
